...
export default function Auth(props) {
	...
	return (
		<Card className='authentication'>
			<h2>{tieneAcceso ? 'Login necesario' : 'Datos de Registro'}</h2>
			<form className='card' onSubmit={handleSubmit(onSubmit)}>
				...
				type='submit'>{tieneAcceso ? 'ENTRAR' : 'REGISTRARSE'} </button>
			</form>
			<button className="buttonR" onClick={gestorDeCambioDeModo}>
				¿Quieres {tieneAcceso ? 'registrarte?' : 'iniciar sesión?'}
			</button>
		</Card>
	);
}
